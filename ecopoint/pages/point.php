...
// คำนวณคะแนนจาก record_points จริง
$total_calculated = 0;
$cup_count = 0;
$bottle_count = 0;
$records_temp = $conn->query("SELECT * FROM record_points WHERE uid = '$uid'");
while($row = $records_temp->fetch_assoc()) {
    $total_calculated += $row['p_total'];
    
    // ตรวจสอบว่า p_other เป็นตัวเลขหรือไม่
    if(is_numeric($row['p_other'])) {
        // p_other เป็นตัวเลข (คะแนนอื่นๆ)
    } else {
        // p_other เป็นข้อความ (เช่น กิจกรรม)
    }
}

// อัพเดทการอ่านแจ้งเตือน
if(isset($_SESSION['username'])){
    $read = $conn->query("UPDATE record_points SET p_noti = 0 WHERE uid = '$uid'");
}

// ใช้คะแนนที่คำนวณได้จาก record_points หากมีข้อมูล
if($history_count > 0 && $total_calculated > 0) {
    $display_point = $total_calculated;
} else {
    $display_point = $total_point;
}
?>
                ...
                <div class="point-display">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="text-start">
                            ...
                            <div class="display-4 fw-bold"><?php echo number_format($display_point); ?></div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-md-6 mb-3">
                <div class="stats-box">
                    <div class="stats-value">
                        <?php 
                        $avg_per_day = $history_count > 0 ? round($display_point / $history_count, 1) : 0;
                        echo $avg_per_day;
                        ?>
                    </div>
                    ...
                    <table class="table table-history">
                        ...
                        <tbody>
                            ...
                            while($data = $sql1->fetch_assoc()): 
                                ...
                                // ตรวจสอบประเภทรายการ
                                $is_activity = (strpos($data['p_other'], 'กิจกรรม') !== false);
                            ?>
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center">
                                ...
                                <td>
                                    <div class="d-flex flex-wrap">
                                        ...
                                        <?php endif; ?>
                                        <?php if(!empty($data['p_other']) && $data['p_other'] != '0'): ?>
                                        <span class="badge-item <?php echo $is_activity ? 'badge-activit' : ''; ?>">
                                            <i class="fas <?php echo $is_activity ? 'fa-calendar-check' : 'fa-tasks'; ?> me-1"></i>
                                            <?php 
                                            // แสดงข้อความจาก p_other
                                            $other_text = $data['p_other'];
                                            // ถ้าเป็นตัวเลขให้แสดงเป็น "อื่นๆ + คะแนน"
                                            if(is_numeric($other_text) && $other_text > 0) {
                                                echo "อื่นๆ +{$other_text}";
                                            } else {
                                                // ตัดคำถ้ายาวเกินไป
                                                echo mb_strlen($other_text) > 20 ? mb_substr($other_text, 0, 20).'...' : $other_text;
                                            }
                                            ?>
                                        </span>
                                        <?php endif; ?>
                                ...
                                <td>
                                    <span class="badge-point <?php echo $is_activity ? 'badge-activit' : 'badge-recycle'; ?>">
                                    ...
                                    <div class="giver-info d-flex align-items-center">
                                        ...
                                        <div>
                                            <div class="fw-bold small">
                                                <?php 
                                                if(!empty($data['p_giver'])) {
                                                    echo htmlspecialchars($data['p_giver']);
                                                } else {
                                                    echo "ระบบ";
                                                }
                                                ?>
